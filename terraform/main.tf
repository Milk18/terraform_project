...
resource "azurerm_virtual_machine_extension" "db_ext" {
  ...
  settings = <<SETTINGS
 {
  "commandToExecute": "sudo apt-get update && sudo apt install git -y && git clone https://github.com/Milk18/terraform_project.git && sudo sh /var/lib/waagent/custom-script/download/0/terraform_project/shell_scripts/db_script.sh"
}
SETTINGS
  depends_on = [
  azurerm_linux_virtual_machine.vm-db
...
resource "azurerm_virtual_machine_extension" "web_ext" {
  ...
  settings = <<SETTINGS
 {
  "commandToExecute": "sudo apt-get update && sudo apt install git -y && git -C ~/ clone https://github.com/Milk18/terraform_project.git && sudo sh ~/terraform_project/shell_scripts/export_script.sh && sudo sh ~/terraform_project/shell_scripts/web_script.sh"
}
SETTINGS
  depends_on = [
  azurerm_linux_virtual_machine.vm-db,
